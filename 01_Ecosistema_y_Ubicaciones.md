# Ecosistema y Ubicaciones — CEB (Centro de Experiencias del Bienestar)

**CEB es la marca paraguas del ecosistema** (confirmado por Edgar 22-07-2026) — no es sinónimo de una sola unidad. Aktiva-T es la marca de las experiencias de movimiento dentro de CEB (Studio y Box), pero CEB es el nombre que engloba todo el ecosistema. Un solo ecosistema de bienestar integral en Coyhaique, no un gimnasio tradicional.

## Las unidades activas (6, dentro de CEB)

1. **Studio Aktiva-T** — entrenamiento guiado, Pilates Reformer, bienestar integral.
2. **Box Aktiva-T** — Cross Training, GAP, alta intensidad grupal.
3. **Cafetería Aktiva** — alimentación consciente.
4. **Patio de la Conexión** — espacio de comunidad y pertenencia (reincorporado como unidad activa el 22-07-2026, tras haber quedado fuera de una versión anterior de la landing).
5. **Coworking CEB** — espacio de trabajo flexible dentro de Baquedano 872. Incorporado el 23-07-2026. Uso individual o reservas para reuniones/talleres/capacitaciones de hasta 20 personas. Incluye aire acondicionado, WiFi de alta velocidad, pantalla interactiva, dirección tributaria, recepción de encomiendas y estacionamiento — con cafetería y gimnasio en el mismo edificio. Reservas por WhatsApp general (+569 42582210). Sin tarifas públicas confirmadas aún.
6. **Recovery — Aktivat Recovery & Performance** — recuperación y regeneración deportiva (descarga muscular, presoterapia, percusión terapéutica). Incorporado el 22-07-2026 como servicio de bienestar (Edgar). Se posiciona como recuperación/rendimiento, NO como clínica médica. Ubicación: Baquedano 872. Planes y precios en [02_Membresias_y_Precios.md](02_Membresias_y_Precios.md).

**Clínica Aktiva-T queda fuera del ecosistema activo por el momento** (su futuro con el negocio es incierto) — no debe aparecer en landing, prompt de Akti, ni material público, aunque la dirección/entrada física siga existiendo (ver más abajo, sección histórica). Nota: Recovery (Aktivat Recovery & Performance) sí es parte del ecosistema activo y es distinto de la Clínica — es un servicio de recuperación deportiva, no atención clínica médica.

## Canales de contacto / WhatsApp
- **General CEB:** +569 42582210 (`wa.me/56942582210`) — usado en web, kiosco, CTAs de la landing. Sede Baquedano 872.
- **Chatbot del gimnasio (Akti):** +569 57196681 (`wa.me/56957196681`) — número reservado para el futuro asistente que deriva a Box o al Studio/CEB según el caso. **Pendiente de desarrollo** — mientras tanto (23-07-2026), el ícono flotante de la landing apunta al WhatsApp general de Baquedano (+56942582210). Cuando el chatbot esté listo, volver a apuntar el ícono flotante a este número.
- **Box Aktiva-T:** +569 73554774. Sede Barroso 777.
- **Cafetería Aktiva:** +569 68390034 (confirmado por Edgar 31-07-2026). Sede Baquedano 872. Publicado en la fila de Cafetería del Ecosistema y en el footer de la landing.
- **Recovery (Aktivat Recovery & Performance):** +569 76187668 (agregado 06-08-2026) — Recovery lo gestiona directamente el profesional a cargo, ya no pasa por el WhatsApp general. Publicado en el botón "Reservar por WhatsApp" del acordeón de Recovery en Membresías y en el footer de la landing.
- **Cumpleaños:** +569 79779242.
- **Correo:** contacto@aktiva-t.com (confirmado 22-07-2026, usado en el footer de la landing).
- **Edgar (número institucional personal):** +569 84039920 (agregado 06-08-2026). Es el número al que el bot de WhatsApp (Meta Business Agent) deriva cuando alguien pide información técnica, quiere conversar sobre su evaluación de composición corporal, o pide hablar directo con un entrenador. No es un número público en la landing — solo lo usa el bot como derivación puntual.

## Actividades adicionales / programas especiales (agregado 22-07-2026)
No son parte de los planes base Esencial/Pro/Box — son programas aparte, sin precio público confirmado aún (pendiente, ver [07_Decisiones_y_Pendientes.md](07_Decisiones_y_Pendientes.md)).

- **Fundamentos** (Studio) — clase grupal de técnica y movimientos fundamentales, previa a musculación o Pilates. Aparte de los planes Esencial y Pro, no incluida en ellos.
- **Clínica formativa de fútbol** (Box) — formación técnica y física a través del fútbol, para niños y jóvenes.
- **Preparación física de equipos** (Box) — programa a medida para equipos/clubes. Requiere completar un formulario de solicitud (aún no implementado como formulario real — hoy deriva a WhatsApp del Box).
- **Funcional Kids** (Box) — clase funcional para niños: juego, movimiento y ejercicio en formato entretenido y guiado. Martes y jueves 18:00. **$39.900/mes** (confirmado por Edgar 23-07-2026).

**Taller de fuerza para niños** — retirado de la landing y del ecosistema público el 23-07-2026 por decisión de Edgar; ya no se ofrece como actividad independiente (Funcional Kids la reemplaza).

## Cumpleaños en Movimiento — en la landing (agregado 22-07-2026)
Servicio conjunto entre **Cafetería Aktiva y el CEB** (Studio). Ya estaba documentado en [05_Servicios_Adicionales.md](05_Servicios_Adicionales.md) — ahora también está publicado en la landing como sección propia (`#cumpleanos`), con los 3 planes (Básico $7.000 p/p, Nutritivo $19.000 p/p, Total $19.000 p/p + inflable) y el WhatsApp de cumpleaños (+569 79779242) como CTA.

## Trabaja con Nosotros — en la landing (agregado 22-07-2026)
Nueva sección (`#trabaja`) con un CTA de correo (`mailto:contacto@aktiva-t.com`) para que quien quiera postular envíe su CV. Enlazada también desde el footer.

### Studio Aktiva-T
- **Dirección:** General Baquedano 872, Coyhaique
- **Qué es:** bienestar integral, entrenamiento guiado, Pilates Reformer, seguimiento profesional, mejora de hábitos, salud preventiva.
- **Instalaciones:** estacionamiento y duchas.
- **Horario:** Lunes a Viernes 06:00–22:00 · Sábado 09:00–13:00. **Único cierre al público:** martes y jueves a las 21:00, reservado para la preparación física de dos equipos de fútbol (confirmado por Edgar 24-07-2026). El resto del horario funciona con normalidad. Publicado en la landing como pestaña propia ("Studio, Musculación y Asistido") dentro de Horarios (sección Membresías).

### Box Aktiva-T (y nota histórica sobre Clínica Aktiva-T)
Box y Clínica funcionan en el mismo edificio/esquina, cada uno con su propio acceso: uno por Almirante Barroso y otro por Cristóbal Colón. **Clínica no forma parte del ecosistema activo actualmente** — se documenta abajo solo como referencia histórica, no para uso en material público.
- **Entrada Box Aktiva-T:** Almirante Barroso 777, Coyhaique.
  - Cross Training y GAP (glúteos, abdomen, piernas). Espacio amplio y acogedor. Alta intensidad con adaptación a cualquier nivel. Clases 100% guiadas por profesores, de calentamiento a elongación final. Comunidad y compañerismo son pilares del lugar.
  - **Horario real (igual todos los días de lunes a viernes):**

    | Día | Mañana | Tarde |
    |---|---|---|
    | Lunes | 08:00–13:00 | 17:00–21:00 |
    | Martes | 08:00–13:00 | 17:00–21:00 (18:00 Funcional Kids) |
    | Miércoles | 08:00–13:00 | 17:00–21:00 |
    | Jueves | 08:00–13:00 | 17:00–21:00 (18:00 Funcional Kids) |
    | Viernes | 08:00–13:00 | 17:00–21:00 |

    Cerrado 13:00–17:00 todos los días. Ya no abre a las 07:00 ningún día. Todos los bloques son "abierto, clase dirigida" salvo donde se indica otra cosa (Funcional Kids martes y jueves 18:00). No abre sábado (no aparece en la grilla).
  - Contacto Box: WhatsApp +569 73554774.
  - Fuente: `Horario_Box_AktivaT_Barroso.pdf` (carpeta `horarios por sede y actividades`, actualizado 04-08-2026) — reemplaza el horario asimétrico anterior (`Horarios actualizados Box Aktiva-T 17 de Julio.jpeg`): ahora es uniforme los 5 días, abre 1 hora más tarde (08:00 en vez de 07:00) y viernes ya tiene el mismo cierre que el resto de la semana.
- **[HISTÓRICO — no usar en material público] Entrada Clínica Aktiva-T:** Cristóbal Colón 203, Coyhaique.
  - Traumatología, kinesiología, nutrición deportiva, pediatría, psicología, tratamiento de la obesidad y reintegro deportivo.
  - Horario: Lunes a Viernes 10:00–13:00 y 15:00–20:00.
  - Excluida del ecosistema activo — ver nota al inicio del documento.

### Cafetería Aktiva
- Alimentación consciente, hábitos saludables.
- **Horario:** Lunes a Viernes 07:00–21:00 · Sábado 08:30–21:00 (confirmado por Edgar 31-07-2026).
- Ubicación: Baquedano 872, junto al Studio.
- **Carta digital:** https://toteat.shop/r/cl/Aktiva/22262/checkin/menu — publicada en la landing como botón "Ver la carta" en la fila de Cafetería del Ecosistema.

### Patio de la Conexión
- Espacio de comunidad y pertenencia, dentro del ecosistema CEB en Baquedano 872.
- Unidad activa (reincorporada 22-07-2026). Falta descripción más detallada de actividades/uso — pendiente que Edgar la complete.

## Horarios detallados por disciplina

### Studio (Baquedano)
- `Horarios Indoor Cycling.docx` — grilla semanal de Indoor Cycling con instructor asignado por bloque.
- **Pilates Reformer Baquedano** — horario ampliado (fuente: `Horarios Pilates reformer Sede Baquedano.pdf`, carpeta `horarios por sede y actividades`, actualizado 04-08-2026, reemplaza `Horarios Pilates Reformer (baquedano).docx`). Alicia Sanhueza dicta prácticamente todos los bloques de lunes a viernes (07:00, 08:15, 09:00, 10:00 mar/jue, 12:15 mar/mié, 13:15 vie, 17:15, 18:15, 19:00, 20:00 mar/jue) más sábado en la mañana (09:00, 10:00, 11:00, 12:00 — sin clase sábado 13:15). **Carla Pérez** dicta dos bloques puntuales en Baquedano: lunes y jueves 13:15–14:00 (antes solo aparecía en Barroso — ver nota en sección de equipo más abajo). Todas las clases duran 45 minutos.

### Sede Barroso / Cristóbal Colón (Box + Clínica, misma esquina)
Pilates Reformer también se dicta en esta sede, con horario propio distinto al de Baquedano, y con un modelo de venta distinto: **plan mensual por cantidad de sesiones**, no incluido en Esencial/Pro. Captura de EVO, semana 12–18 julio 2026:

| Clase | Días | Horario | Instructora |
|---|---|---|---|
| Pilates reformer | Lunes, Miércoles, Viernes | 09:00–09:45 | Carla |
| Pilates adulto mayor | Martes, Jueves | 10:00–10:45 | Carla |
| Pilates reformer | Lunes, Miércoles, Viernes | 18:00–18:45 | Antonia |

> Grilla capturada desde el sistema de reservas (EVO), no es un archivo fuente — confirmar vigencia si pasan varias semanas.

**Planes y precios (confirmados por Edgar 23-07-2026, publicados en la landing):**
- Pilates Reformer, 12 sesiones/mes: **$120.000/mes**
- Pilates Adulto Mayor, 8 sesiones/mes: **$64.000/mes**

## Contacto
- WhatsApp general CEB: usado en la web/kiosco (`wa.me/56942582210`).
- Box Aktiva-T: +569 73554774 (distinto del WhatsApp general — confirmar si ambos deben usarse o si el del Box es el vigente para consultas de esa unidad).
- Cumpleaños: +569 79779242.

## Cómo elegir la unidad correcta (criterio de venta)
- **Studio Aktiva-T** si busca: salud, entrenar guiado a su ritmo, Pilates, acompañamiento, empezar desde cero, recomposición corporal, bienestar.
- **Box Aktiva-T** si busca: intensidad, comunidad, Cross Training, funcional, rendimiento, menor inversión mensual, clases de GAP.
- **Cafetería Aktiva**, **Patio de la Conexión** y **Coworking CEB** no se "eligen" como alternativa a entrenar — son parte transversal de la experiencia CEB, complementan cualquier camino (Studio o Box).

Si el objetivo es ambiguo (ej. "bajar de peso"), se pregunta el estilo: "¿Prefieres entrenar a tu ritmo con guía profesional, o en grupo con energía e intensidad?" (a tu ritmo → Studio; grupo/intensidad → Box).

No mencionar Clínica en este criterio — está excluida del ecosistema activo (ver nota al inicio del documento).

## Equipo (publicado en la landing, sección "El equipo", agregado 23-07-2026, ubicaciones corregidas 24-07-2026)

Por defecto todo el equipo trabaja en **CEB Baquedano 872 (Studio)**, salvo Jonathan y Carla (Box) y Antonia (Pilates Reformer Barroso). El filtro de la sección "El equipo" en la landing tiene 4 categorías: Studio, Box, Pilates Baquedano, Pilates Barroso (24-07-2026) — una persona puede aparecer en más de una si corresponde (ej. Carla es Box y también Pilates Barroso).

- **Fidel Pinilla** — Director CEB (aprueba descuentos especiales y convenios institucionales, ver [02_Membresias_y_Precios.md](02_Membresias_y_Precios.md) y [04_Convenios_Corporativos.md](04_Convenios_Corporativos.md)). Transversal a todo el ecosistema.
- **Edgar Valenzuela** — Entrenador, Baquedano. A cargo de Fundamentos.
- **Felipe Castillo** — Entrenador, Baquedano.
- **Jonathan Ulloa** — Entrenador, **Box Aktiva-T (Barroso)**.
- **Francisco Urbina** — Entrenador, Baquedano.
- **Camilo Yáñez** — Entrenador, Baquedano.
- **Jose Urbina** — Entrenador, Baquedano.
- **Carla Pérez** — Entrenadora, **Box Aktiva-T (Barroso)**. También dicta Pilates Reformer y Pilates Adulto Mayor en **Barroso** (ver horarios en sección Barroso más abajo) — aparece en el filtro Box y Pilates Barroso. **Pendiente de confirmar con Edgar (detectado 04-08-2026):** el nuevo horario de Pilates Reformer Baquedano (`Horarios Pilates reformer Sede Baquedano.pdf`) también la muestra dictando lunes y jueves 13:15–14:00 en Baquedano — evaluar si corresponde agregarle también el tag "pilates-baquedano" en el filtro de la landing, o si es un reemplazo puntual de Alicia y no amerita cambiar su ficha.
- **Alicia Sanhueza** — Instructora de Pilates, **Baquedano** — aparece en el filtro Studio y Pilates Baquedano.
- **Antonia Campos** — Instructora de Pilates, **Barroso** (Pilates Reformer sede Barroso) — aparece solo en el filtro Pilates Barroso.
- **Diego Aravena** — Entrenador, Baquedano.
- **Felipe Arroyo** — Entrenador, Baquedano.
